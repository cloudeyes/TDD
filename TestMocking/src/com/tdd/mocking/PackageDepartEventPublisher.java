...
public class PackageDepartEventPublisher {
  ...
  public PackageDepartEventPublisher(
      final AmazonSNS snsClient, final AmazonSNSIdentifier snsIdentifier) {
  ...
  public PublishResult publishEvent(final PackageDepartEvent packageDepartEvent) {
    ...
    checkArgument(
        !Strings.isNullOrEmpty(packageDepartEvent.getEventMessage()),
    ...
    try {
      result =
          snsClient.publish(snsIdentifier.getTopic(), eventMessage, snsIdentifier.getSubject());
      ...
      throw new DepartNotificationPublishException(
          "Error returned by SNS: " + exception.toString(), exception);
    }
